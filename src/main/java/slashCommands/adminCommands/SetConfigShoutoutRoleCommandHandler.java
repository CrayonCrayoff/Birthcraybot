...
public class SetConfigShoutoutRoleCommandHandler implements SlashCommandInterface {
    @Override
    public String getName() {
        return "set-config-shoutout-role";
    }

    @Override
    public CommandData getCommandData() {
        return Commands.slash(this.getName(), "Set the shoutout role")
                .setContexts(InteractionContextType.GUILD)
                .setDefaultPermissions(DefaultMemberPermissions.DISABLED)
                .addOption(
                        OptionType.STRING,
                        "shoutout-role",
                        "Set the role to notify other people about someone's birthday (leave empty to remove)",
                        true,
                        true
                );
    }
    ...
    public void executeCommand(SlashCommandInteractionEvent event) {
        String roleId = event.getOption("shoutout-role").getAsString();
        Role role;
        try {
        ...
        try {
            Database.setShoutoutRole(event.getGuild().getId(), roleId);
            event.getHook()
                    .editOriginal("Birthcraybot shoutouts will ping users with the role " + role.getName())
                    .queue();
